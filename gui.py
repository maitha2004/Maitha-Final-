...
# CODE FOR UI ACTIONS# Employee UI Class
class EmployeeForm:
    @staticmethod
...
class MainApp:
    ...
    def refresh_employee_view(tree):
        ...
        for employee in employees:
            tree.insert('', 'end', values=(employee.employee_id, employee.name, employee.department, employee.job_title, employee.basic_salary))
    # Entries
    ...
    def create_employee_tab(self):
        def show_employees():
            ...
            delete_button = tk.Button(employees_window, text="Delete Employee", command=lambda: MainApp.delete_employee(tree))
            delete_button.pack()
        ...
        self.name_entry, self.id_entry, self.department_entry, self.title_entry, self.salary_entry, self.manager_id_entry, tree))
    ...
    # Input fields and labels for event details
     ...
     tk.Button(event_input_frame, text="Add Event", command=EventForm.add_event(self.event_id_entry, self.event_type_entry, self.theme_entry, self.date_entry, self.time_entry, self.duration_entry, self.venue_entry, self.client_entry, self.suppliers_entry )).grid(row=9, columnspan=2)
    ...
    def create_client_tab(self):
     def show_clients():
        ...
        list_frame.pack(padx=10, pady=10)
        tree = ttk.Treeview(list_frame, columns=("ID", "Name", "Address", "Contact Details", "Budget"), show="headings")
        ...
        delete_button = tk.Button(clients_window, text="Delete Client", command=lambda: ClientForm.delete_client(tree))
     ...
     tk.Button(client_input_frame, text="Add Client", command=lambda: ClientForm.add_client(client_window, client_id_entry, client_name_entry, client_address_entry, client_contact_details_entry, client_budget_entry)).grid(row=5, columnspan=2)
    ...
    def create_supplier_tab(self):
     def show_suppliers():
        ...
        delete_button = tk.Button(suppliers_window, text="Delete Supplier", command=lambda: SupplierForm.delete_supplier(tree))
        delete_button.pack()
     ...
     tk.Button(supplier_input_frame, text="Add Supplier", command=lambda: SupplierForm.add_supplier(supplier_id_entry, name_entry, supplier_address_entry, supplier_contact_details_entry, tree)).grid(row=4, columnspan=2)
    ...
    def create_guest_tab(self):
     ...
     tk.Button(guest_input_frame, text="Add Guest", command=lambda: GuestForm.add_guest(guest_id_entry, name_entry, guest_address_entry, guest_contact_details_entry, tree)).grid(row=4, columnspan=2)
    ...
    def create_venue_tab(self):
     def show_venues():
        ...
        list_frame.pack(padx=10, pady=10)
        tree = ttk.Treeview(list_frame, columns=("ID", "Name", "Address", "Contact Details", "Min Guests", "Max Guests"), show="headings")
     ...
     tk.Button(venue_input_frame, text="Add Venue", command=lambda: VenueForm.add_venue(venue_id_entry, name_entry, venue_address_entry, venue_contact_details_entry, venue_min_guests_entry, venue_max_guests_entry, tree)).grid(row=6, columnspan=2)
